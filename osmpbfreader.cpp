                  ...
                  if (pg.nodes_size() > 0) {
                      ...
                      for (int i = 0; i < maxnodes; ++i) {
                          ...
                          for (int k = 0; k < pg.nodes(i).keys_size(); ++k) {
                              const char *ckey = primblock.stringtable().s(pg.nodes(i).keys(k)).c_str();
                              if (strcmp("name", ckey) == 0) {
                                  const uint64_t id = pg.nodes(i).id();
                                  const bool result = swedishTextTree->insert(primblock.stringtable().s(pg.nodes(i).vals(k)),  id << 2 | NODE_NIBBLE);
                                  if (!result)
                                      Error::warn("Cannot insert %s", primblock.stringtable().s(pg.nodes(i).vals(k)).c_str());
                              }
                          }
                      }
                  }
