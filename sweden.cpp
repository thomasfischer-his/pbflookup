...
void Sweden::insertWayAsRoad(uint64_t wayid, RoadType roadType, uint16_t roadNumber) {
    ...
    static const uint64_t blacklistedWayIds[] = {1648176, 1648475, 1651992, 2954124, 4605570, 8150233, 9702586, 23275365, 23444254, 23444292, 23444785, 23538748, 24040916, 24731206, 24731243, 24786276, 24838509, 25380767, 25459932, 25953242, 26205243, 27565380, 27872415, 27872417, 27872418, 29054792, 29054793, 30784964, 30887520, 34419027, 34419029, 37627542, 38227481, 38564589, 38564590, 42798132, 44141405, 44298775, 45329454, 45876899, 46931166, 48386475, 51381476, 51385960, 53184762, 59065373, 59065380, 59065382, 59065388, 60928268, 61380105, 62080181, 64173539, 67171996, 69358305, 73854172, 80360747, 116831322, 121821747, 127023670, 138001545, 138003259, 146294832, 180751968, 180987048, 194028774, 211946632, 216938864, 218214593, 225436893, 229700851, 257700608, 258575332, 258575333, 258575334, 298780382, 298884852, 308918468, 308918469, 315471029, 321318578, 324044848, 324093398, 324093732, 324201518, 324271180, 324492881, 324492887, 326365472, 345614344, 345614345, 347763180, 347763181, 347763182, 347763184, 347763185, 347763186, 347763188, 366707779, 375573546, 375573548, 375671059, 375671060, 375671285, 375671286, 375671287, 383462866, 394305929, 394305930, 399732015, 402989392, 413609159, 417220648, 417220650, 0};
    /// To count: echo '3, 1, 2' | sed -e 's/ //g' | tr ',' '\n' | wc -l
    static const size_t blacklistedWayIds_count = 114;
    if (inSortedArray(blacklistedWayIds, blacklistedWayIds_count, wayid)) return;
...
Sweden::RoadType Sweden::closestRoadNodeToCoord(int x, int y, const Sweden::Road &road, uint64_t &bestNode, int &distance) const {
    ...
    if (wayIds != nullptr) {
        ...
        if (minSqDistance < INT64_MAX && bestNode > 0) {
            ...
            Error::debug("Closest node of road %s to x=%d,y=%d is node %llu at distance %.1f km", road.operator std::string().c_str(), x, y, bestNode, distance / 1000.0);
        } else
            bestNode = 0;
    }
